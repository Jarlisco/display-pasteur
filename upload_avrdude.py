...
def usb_callback(*args, **kwargs):
    print("USB the AVR board")
    global host, port,device
    env.Execute("avrdude -C /etc/avrdude/avrdude.conf -c arduino -p "+device+" -P/dev/ttyACM0 -b115200 -D -Uflash:w:.pio/build/ATmega328P/firmware.hex")

def upload_callback(*args, **kwargs):
    print("Telnet the AVR board")
    global host, port,device
    env.Execute("/usr/bin/avrdude -C /etc/avrdude/avrdude.conf -c arduino -p "+device+" -P net:"+host+":"+port+" -U flash:w:.pio/build/ATmega328P/firmware.hex")
...
def assembly_callback(*args, **kwargs):
    ...
    piohome = "/home/agicquel/.platformio/packages"
    ...
    envOptions="-Iinclude -Isrc -I.pio/libdeps/ATmega328P/HardWire_ID1437/src -I{0}/cores/MiniCore -I{0}/variants/standard -I{0}/libraries/Wire/src".format(binEnv)
...
env.AlwaysBuild(env.Alias("usbupload", None, usb_callback))
